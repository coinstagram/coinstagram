...
router.get("/relationship/followee", async (req, res) => {
  ...
  try {
    const connection = await pool.getConnection(async (conn) => conn);
    try {
      sql = `select user_id, user_name from users where id in(select followee_id from users_relationship where follower_id = ?);`;
...
router.get("/relationship/follower", async (req, res) => {
  ...
  try {
    const connection = await pool.getConnection(async (conn) => conn);
    try {
      sql = `select user_id, user_name from users where id in(select follower_id from users_relationship where followee_id = ?);`;
